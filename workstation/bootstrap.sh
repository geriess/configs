...
ExecStart=/usr/bin/docker run -h dev -e TZ=America/Chicago -p 1313:1313 --net=host --rm -v /var/run/docker.sock:/var/run/docker.sock -v /mnt/code:/root/code -v /mnt/secrets:/root/secrets --cap-add=SYS_PTRACE --security-opt seccomp=unconfined --privileged --name dev geries/dev:latest
